...
public class InterviewController implements InterviewApi {
    ...
    public ResponseEntity<InterviewAnswerWithCommentsResponse> getInterviewAnswer(
            @PathVariable("interviewAnswerId") Long interviewAnswerId) {
        Long memberId = SecurityUtil.getAuthenticatedMemberId();
        InterviewAnswerWithCommentsResponse interviewAnswerWithCommentsResponse = interviewService.getInterviewAnswer(
                memberId, interviewAnswerId);

        return ResponseEntity.status(HttpStatus.OK)
                .body(interviewAnswerWithCommentsResponse);
    }
